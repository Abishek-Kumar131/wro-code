...
/*
  ...
  - Failsafe: the motor stops if no command arrives for 500 ms (applies during timed
    arc turns too).
  - Steering is limited to 70-130 degrees (+-30). Past ~30 deg the tyres skid.
  - Telemetry to the Pi:
...
//#---Servo Angles (+-30 deg usable steering range)---##################################
int servo_center = 100;                  // 100 deg (Straight center)
int left_turn_angle = servo_center - 30; // 70 deg (full left)
int right_turn_angle = servo_center + 30;// 130 deg (full right)
