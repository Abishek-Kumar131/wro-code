...
Includes stuck recovery: if the car wedges itself against a wall it backs out and
carries on instead of sitting there grinding the wheels. Use --no-stuck-recovery to
detect it but never reverse.
...
How it gets out
  ...
  the turns is what decides the escape.
...
Usage
  ...
  python3 open_challenge_R1.py --no-stuck-recovery   report a wedged car but never reverse
        ...
        while True:
            ...
            if status_to_terminal and now - last_status > 0.2:
                ...
                print(f"\r{state:8s} t={turns:2d} L={left_area:5d} R={right_area:5d} "
                      f"O={orange_area:4d} B={blue_area:4d} F={front_area:5d} ang={angle:3d} mot={stuck.motion:5.2f} "
